...
```{r, echo = FALSE}
(plot_resid(fit, type = "pearson")) + (plot_resid(fit2, type = "pearson"))
```
...
The quasi-poisson model estimates the same parameter and adjust standard errors as we did on slide XXXX. All other parameters are the same.
...
Here we compare the expected variance ($\pm 2\sqrt{V(y_i)}$) of the Quasi-Poisson and Poisson models:
...
Where $\theta$ is the overdispersion parameter, $\Gamma()$ is the gamma function. The mean is $\mu$ and the variance is $\mu + \frac{\mu^2}{\phi}$. The $\theta$ is the inverse of the overdispersion in terms that as $1/\theta$ increase the data are more overdispersed. As $1/\theta$ approaches 0, the negative-binomial reduced to a Poisson model.
...
```{r}
plot(ggeffect(fit_glm, terms = c("x", "g"))) | plot(ggeffect(fit_lm, terms = c("x", "g")))
```
