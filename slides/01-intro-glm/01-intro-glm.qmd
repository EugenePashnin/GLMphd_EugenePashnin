...
This part is invariant to the type of model and is the combination of explanatory variables to predict the expected value $\mu$ (i.e. mean) of the distribution.
...
linktab <- data.frame(Family = code(fam), Link = link, Range = range)
linktab |> 
  mytab() |> 
  cat()
```
...
The expected value and variance are defined as:
